...
export function MobileMenu({ isOpen, onClose, triggerRef }: MobileMenuProps) {
  ...
  // Close only when pathname changes (not on mount or when isOpen toggles)
  ...
  // ESC to close (immediate, no animation)
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* Drawer - stopPropagation so clicks inside don't close */}
      <div
        ref={drawerRef}
        role="dialog"
        aria-modal="true"
        ...
        className={cn(
          "fixed top-0 right-0 z-50 h-full w-[75%] max-w-sm",
